...
- В правом нижнем углу оболочки закреплена отдельная кнопка чата с отступами 12 px. Она открывает модальное окно поверх текущего таба. В окне есть независимые режимы `Генерация / Задать вопрос`; по умолчанию выбран `Задать вопрос`. История режимов хранится раздельно. В режиме вопросов дизайнер может переключить источник между `LangFlow` и `Локальный Codex`; выбор сохраняется в `figma.clientStorage`, а локальный режим передаёт ограниченную историю диалога в read-only Codex-контур. Вкладки `Паттерны` и `Тексты` при этом не меняют режим работы.
- Режим `Генерация` отправляет промпт, неизменяемый исходный brief, номер раунда и ограниченную историю уточнений только в `/v1/generation/plan` Apollo Proxy. Read-only агент Proxy использует `ds-ai-hub` как источник паттернов, правил и component cookbooks и возвращает semantic plan без Figma keys. Затем Proxy разрешает published component/variable keys, допустимые варианты и semantic targets исключительно через `design-system_ab`. Плагин принимает закрытую схему `apollo.generation-plan.v5`, импортирует компоненты и Variables, применяет подтверждённые properties/text overrides и собирает Auto Layout на текущей странице. `preconditionOverrides` временно переводят вложенные instances в редактируемое состояние (например, открывают FAQ item для записи ответа), после чего финальные `instancePropertyOverrides` восстанавливают состояние из брифа. Если не хватает продуктового решения, Proxy возвращает `clarification_required`; технический тупик или повтор вопроса завершается terminal `blocked` без изменения Figma. Для desktop landing Proxy дополнительно применяет детерминированные compiler defaults: `CorporateContent → slots.body`, bento PromoCard 3+2 и base-surface footer; preflight проверяет состав, количества, inline-form, FAQ и точный CTA до исполнения. Созданный корень выделяется; вся операция отменяется штатным Undo Figma.
- В режиме генерации UI запрашивает `/health` и показывает версию подключённого Apollo Proxy. Это позволяет отличить актуальную сборку от оставшегося запущенным старого приложения.
- Runtime генерации расположен в [`src/generation`](./src/generation) и имеет отдельные message types, request id и состояние. Он не читает `lastApollo*Report`, не вызывает `runAudit`, `scanSelection` или Predicate Engine и при ошибке удаляет частично созданный корень. Поэтому генерация не меняет результаты, lifecycle или очередь проверки макетов.
- Вкладка `Паттерны` работает на production Predicate Engine. Она отправляет неизменяемый EvidenceBundle v2 в `/v1/validate/predicates`; proxy сам выбирает подписанный release и компилирует только исполняемые правила пакетов, присутствующих в снимке, а также применимые страничные контуры. Клиент не передаёт `ruleSet` и не может добавить или изменить правило. Evidence graph включает наблюдаемые `opacity` и `radius`; baseline evidence объединяет точный WIP diff с полной категорией кастомизаций, чтобы component-owned правила могли повторно оценить даже предварительно допустимые изменения вложенных компонентов. Generic baseline-контур использует только точный baseline, а при override без него возвращает `Зови ДС`, не подменяя baseline визуально похожим значением. Агент не участвует в verdict и не может добавить, удалить или изменить строку.
- Граница production/legacy, инварианты source-only исполнения и обязательные gates перед Figma-тестом зафиксированы в [`docs/PREDICATE_PRODUCTION_CONTOUR.md`](./docs/PREDICATE_PRODUCTION_CONTOUR.md).
