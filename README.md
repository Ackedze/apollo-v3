...
- Вкладка `Паттерны` работает на production Predicate Engine. Она отправляет неизменяемый EvidenceBundle v2 в `/v1/validate/predicates`; proxy сам выбирает подписанный release и компилирует только исполняемые правила пакетов, присутствующих в снимке, а также применимые страничные контуры. Клиент не передаёт `ruleSet` и не может добавить или изменить правило. Evidence graph включает наблюдаемые `opacity` и `radius`; generic baseline-контур использует точный WIP diff, а при override без точного baseline возвращает `Зови ДС`, не подменяя baseline визуально похожим значением. Агент не участвует в verdict и не может добавить, удалить или изменить строку.
